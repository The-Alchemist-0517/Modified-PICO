...
class MAPFEnv(gym.Env):
    ...
    # Initialize env
    def __init__(self, num_agents=1, observation_size=3,world0=None, goals0=None, DIAGONAL_MOVEMENT=False, SIZE=(10,40), PROB=(0,.5),
    ...
    def _step(self, action_input,episode=0,output_priority=False):
        ...
        # on_goal estimation
        self.on_goal = self.world.getPos(agent_id) == self.world.getGoal(agent_id)
        
        # Unlock mutex
        self.mutex.release()
        if output_priority:
            priority = self.world.getPriority(agent_id)
            return state, reward, done, nextActions, self.on_goal, blocking, valid_action, priority

        return state, reward, done, nextActions, self.on_goal, blocking, valid_action
